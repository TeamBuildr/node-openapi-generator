...
const createTestFactory = async (hbsPath, outDirPath, key, openapispec) => {
  ...
  Object.keys(modelDef.properties).forEach((property) => {
    ...
    propObj.fakerArray = propObj.dataType.startsWith('[{');
    propObj.fakerInt = propObj.dataType === '{Integer}';
    propObj.fakerNumber = propObj.dataType === '{Number}';
    propObj.fakerString = propObj.dataType === '{String}';
  });
...
module.exports = (hbsPath, outDirPath, key, openapispec) => createTestFactory(hbsPath,
  outDirPath,
  key,
  openapispec);
